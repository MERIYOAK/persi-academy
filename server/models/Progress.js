...
progressSchema.methods.updateVideoProgress = function(watchedDuration, totalDuration, timestamp = null) {
  ...
  // Update course-level completion percentage
...
progressSchema.statics.getOverallCourseProgress = async function(userId, courseId) {
  ...
  const courseProgressPercentage = Math.round(totalCompletionPercentage / totalVideos);
...
progressSchema.statics.getCourseProgressSummary = async function(userId, courseId) {
  const progressEntries = await this.find({ userId, courseId });
